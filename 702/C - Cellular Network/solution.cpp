...
bool check( const vll& arr,ll r,ll m,const vll& trr){
    ll j=0;
    for(ll i=0;i<arr.size();i++){
        while(j<m && trr[j] <arr[i]-r){j++;}
        if(j==m ||trr[j]>arr[i]+r){return false;}
    }
    return true;
}
void solve() {
    ll n,m; cin>>n>>m; vll arr(n),trr(m);
    for(ll i=0;i<n;i++){ cin>>arr[i];}
    for(ll i=0;i<m;i++){ cin>>trr[i];}
    ll l=-1;
    ll r=2*1e9;
    while(r-l>1){
        ll mid=l+(r-l)/2;
        check(arr,mid,m,trr)?r=mid:l=mid;
    }
    cout<<r<<endl;
 
 
