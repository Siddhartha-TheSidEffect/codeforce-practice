...
// Macro for column-oriented vector declaration and transposed input
...
void solve() {
    ...
    ll r=n-1;
    while(l<=r){
        if(arr[l]==mini){l++; mini++;}
        else if(arr[l]==maxi){l++; maxi--;}
        else if(arr[r]==maxi){r--;maxi--;}
        else if(arr[r]==mini){r--;mini++;}
        else{
            cout<<l+1<<" "<<r+1<<endl;
            return;
        }
