...
 
void solve() {
    ll n; cin>>n; vpll arr1(n),arr2(n),arr3(n);
    for(ll i=0;i<n;i++){ ll x; cin>>x; arr1[i]={x,i};}
    for(ll i=0;i<n;i++){ ll x; cin>>x; arr2[i]={x,i};}
    for(ll i=0;i<n;i++){ ll x; cin>>x; arr3[i]={x,i};}
    ll ans=0;
    rsrt(arr1);
    rsrt(arr2);
    rsrt(arr3);
    for(ll i=0;i<3;i++){
        for(ll j=0;j<3;j++){
            for(ll k=0;k<3;k++){
                ...
                ll b=arr2[j].first;
                ll c=arr3[k].first;
                if(arr1[i].second !=arr2[j].second && arr2[j].second != arr3[k].second &&arr1[i].second !=arr3[k].second){
                    ans=max(ans,a+c+b);
                }
 
 
