...
 
void solve() {
    ll n,k; cin>>n>>k; vll arr(n);
    for(ll i=0;i<n;i++){ cin>>arr[i];}
    vll crr[k+1];
    for(ll i=1;i<=k;i++){crr[i].pb(0);    }
    for(ll i=0;i<n;i++){crr[arr[i]].pb(i+1);}
    for(ll i=1;i<=k;i++){crr[i].pb(n+1);}
    priority_queue<ll>pq[k+1];
    ll ans=LLONG_MAX;
    for(ll i=1;i<=k;i++){
        for(ll j=0;j<crr[i].size()-1;j++){
            pq[i].push(crr[i][j+1]-crr[i][j]-1);
        }
        ll mx=pq[i].top();
        pq[i].pop();
        if(mx%2==0){
