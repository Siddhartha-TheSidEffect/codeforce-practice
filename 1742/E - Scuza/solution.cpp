...
 
void solve() {
    ll n,q; cin>>n>>q; vll arr(n),qrr(q),prefix(n+1,0);
    for(ll i=0;i<n;i++){ cin>>arr[i];}
    for(auto &x:qrr){cin>>x;}
    ...
    ll temp2=arr[0];
    for(ll i=1;i<n;i++){
        temp2=max(arr[i],temp2);
        if(arr[i]<arr[i-1]){arr[i]=temp2;}
    }
    vll ans;
    for(ll i=0;i<q;i++){
        ll temp=upper_bound(arr.begin(),arr.end(),qrr[i])-arr.begin();
        if(temp==n){
            ans.pb(prefix[temp-1]);
        }
        else if(arr[temp]==qrr[i]){
            ans.pb(prefix[temp]);
        }
        else if(temp>0 &&temp<n){ans.pb(prefix[temp-1]);}
        else{ans.pb(0);}
 
    }
